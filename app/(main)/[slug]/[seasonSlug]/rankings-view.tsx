...
export function RankingsView({
  ...
  const currentSeason =
    seasons.find((s) => s.slug === currentSeasonSlug) ?? null;
  ...
  useEffect(() => {
    if (challengeFromUrl) {
      window.history.replaceState(
        null,
        "",
        routes.season(clubSlug, currentSeasonSlug),
      );
    }
  }, [challengeFromUrl, currentSeasonSlug, clubSlug]);
  ...
  function handleNavigateToPlayer(player: {
    ...
    if (player.playerSlug) {
      router.push(routes.player(player.playerSlug));
    }
    ...
    <div className="flex items-center gap-2">
      ...
      {seasons.length > 1 && (
        <SeasonSelector
          ...
          onChange={(slug) => router.push(routes.season(clubSlug, slug))}
        />
      )}
    </div>
  );
  ...
  function handleMatchClick(id: number) {
    router.push(routes.match(clubSlug, currentSeasonSlug, id));
  }
