...
/club/[id]                        → Club page (info, seasons, members)
...
└──────────────────────────────┘
```
- `☰` (left): Opens fullscreen hamburger overlay (→ US-NAV-01)
- Club name (center): Tappable, navigates to `/club/[id]` (→ US-CLUB-01)
- 🔔 (right): Bell with unread badge

**Hamburger Menu (mobile)** — fullscreen overlay opened by ☰:
```
┌──────────────────────────────┐
...
Tapping a nav item closes the overlay and navigates to the selected route. Tapping outside or swiping closes the overlay.

**Challenge FAB (mobile)** — floating action button, bottom-right, `court-500`, raised with shadow. Visible on rankings and matches pages. Tapping opens the quick challenge flow.

**Sidebar Nav (desktop, lg+)** — persistent left sidebar:
...
Club name at top of sidebar is tappable → navigates to `/club/[id]`.

**Club Switcher** — inside hamburger overlay (mobile) and top of sidebar (desktop). Shows all clubs the player belongs to. Switching changes the context for rankings, matches, and the FAB challenge flow. Feed always shows cross-club content but has club filter tabs.
...
```
Player taps FAB or "Challenge" button on player profile
        ↓
...
- Card variants: current player (`court-500`), challengeable (subtle `court-50/50` + `court-300` ring), challenged (`orange-50` + `orange-400` ring), unavailable (dimmed).
- Tap any player → navigates to player profile (`/player/[id]`). Challenges are initiated from the profile page or the FAB.
- Names are abbreviated in pyramid cards: "Raphael Gruber" → "Raphael G."
...
**Triggered by**: Floating FAB (bottom-right on mobile) or "Challenge" button in page header (desktop).
...
**Access**: Visible to players with `admin` role in the club. Nav item "Admin" appears in sidebar (desktop) and hamburger menu (mobile) only for admins.
...
| Hamburger menu / sidebar | "Admin" nav item appears |
...
|---------|---------------|---------------|
| Navigation | Hamburger menu (☰) + top bar | Sidebar (left) |
