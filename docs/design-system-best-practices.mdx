...
Principles for building polished, consistent UIs. General-purpose reference applicable to any project, with a Pyramid-specific audit at the end.
...
| **Toast** | Simple confirmation, no action needed | 3-5 seconds, auto-dismiss | "Challenge sent" |
| **Snackbar** | Confirmation with optional undo/action | 5-8 seconds | "Match withdrawn. [Undo]" |
| **Banner** | Important status affecting the whole page | Persistent until dismissed | "Season ended. View final standings." |
| **Dialog** | Requires immediate decision | Until user responds | "Confirm forfeit?" |
...
- **Be specific, not generic**: "Unable to load rankings — the server is unreachable" beats "Something went wrong."
...
- **Breakpoints mark structural changes, not tweaks**: A breakpoint should switch layout patterns (stack → side-by-side, bottom nav → sidebar), not adjust padding by 4px.
...
|---------|--------|---------|
| Navigation | Bottom bar (thumb-friendly) | Sidebar (visible labels) |
...
- **Color is supplementary, never primary**: Use icons, labels, patterns, or shapes alongside color. A green badge should say "Win" or include a checkmark — never rely on green alone.
...
- **Sentence case for UI labels**: Sentence case ("Create new season") feels approachable. Title case ("Create New Season") feels formal. Pick one and apply it everywhere.
- **Button labels are verbs**: "Save changes", "Send challenge", "Sign in" — not "OK", "Submit", or "Continue" (which are ambiguous).
...
- **Truncation rules**: Define per component — max lines, ellipsis behavior, tooltip on overflow. Never let truncation hide critical information (e.g., truncating a player name mid-word is better than hiding their rank).
...
| Button labels | Verb + object ("Save changes", "Send challenge") |
...
- **Consistent color mapping**: If green means "wins" in one chart, it means "wins" in every chart. Never reuse a color for a different variable within the same view.
...
- Code references semantic tokens, not primitives. Changing `color-action-primary` from `green-500` to `blue-500` updates every button in the system.
...
- Names describe purpose, not value: `color-action-primary` not `green-500`
...
## 15. Pyramid "Court" Audit

Mapping each principle against the current Court design system.

### What's Strong

| Area | Current state |
|------|--------------|
| **Color palette** | Well-defined: `court-*`, `trophy-*`, `slate-*` with clear semantic roles. No gray/zinc leakage. |
| **Typography** | Inter with defined scale, `tabular-nums` for scores, tight tracking on headings. |
| **Spacing** | 4px base unit, documented per-component spacing values. |
| **Component architecture** | Three-layer system (primitives → composites → domain) with strict import rules. |
| **Composition rules** | Enforced: DataList for all lists, ResponsiveDialog for all modals, FormField for all forms, ConfirmDialog for destructive actions. |
| **Border radius strategy** | Consistent: `rounded-xl` for interactive, `rounded-2xl` for cards, `rounded-full` for badges. |
| **Elevation** | Ring-based borders (no layout shift), shadow reserved for floating elements. |
| **Dark mode** | Light-first with documented light/dark token pairs. Correct: soft dark backgrounds, not pure black. |
| **Motion** | Defined durations and easing curves. Framer Motion for layout animations. |
| **Accessibility** | Storybook a11y addon, documented checklist for new components/stories, touch targets enforced. |
| **Responsive** | Two-tier mobile/desktop with documented breakpoint behavior per component. |
| **Content strategy** | English code, German UI via `next-intl`. Translation keys enforce separation. |

### Gaps & Recommendations

| Principle | Gap | Recommendation |
|-----------|-----|----------------|
| **Interaction states** | No documented state matrix for components. States exist in code but aren't specified as a design deliverable. | Add a "Component States" section to `design-system.mdx` documenting hover, focus, active, disabled, loading for each component type. |
| **Edge states** | Empty states and loading states are enforced via `DataList`/`EmptyState`/`Skeleton`, but no design guidelines for *content* (what to write, what icon to use, what CTA). | Add empty state content guidelines: per-page first-use, no-results, and user-cleared copy specs. |
| **Error state design** | `error.tsx` exists as a convention, but no design system guidance on error message tone, placement, or recovery patterns. | Document error state patterns: inline field errors, page-level errors, toast errors, with copy guidelines. |
| **Feedback patterns** | Toast/notification patterns not yet documented as a design system concern (only mentioned in UI spec). | Consolidate feedback patterns (toast, snackbar, banner, inline) into the design system with when-to-use guidelines. |
| **Focus indicators** | `color-contrast` disabled globally in a11y testing. Focus ring design not explicitly documented. | Design and document focus indicator appearance: color, width, offset for each component type. Enable contrast testing once finalized. |
| **Content/microcopy** | German UI text is in translation files, but no documented tone, voice, or copy conventions. | Add a content guidelines section: sentence case rules, button label patterns, error message tone, truncation rules. |
| **Iconography** | "Heroicons 2 only, `size-5` for UI, `size-6` for nav" is defined. Stroke weight, optical alignment, and icon-label pairing rules are not. | Expand icon guidelines with stroke weight consistency requirements and icon-label pairing rules. |
| **Data visualization** | Rank progression chart in the UI spec, but no design system guidance for chart colors, axis labels, or accessibility. | Add data viz guidelines covering the rank chart: color usage, axis labeling, text alternatives. |
| **Reduced motion** | Motion tokens are defined but no `prefers-reduced-motion` strategy documented. | Document reduced motion alternatives for each animation type. |
| **Spacing hierarchy** | Individual spacing values documented, but the *relationship* between page padding > section gap > card padding > element gap is not visually documented. | Add a spacing hierarchy diagram showing how the levels relate. |
