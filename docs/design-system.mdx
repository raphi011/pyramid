...
- **Bottom navigation bar**: 2 items (Rankings, Feed) + center FAB
...
Profile is accessed via the sidebar (desktop) or avatar link, not as a bottom nav item.
...
```tsx
<BottomNav items={[
  { icon: TrophyIcon, label: "Rankings", href: "/rankings" },
  { icon: SwordsIcon, label: "Matches", href: "/matches" },
  ...
]} />
```
