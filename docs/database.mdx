...
| `image_id` | `UUID` | `FK → images(id)` | Optional inline image. Served via `/api/images/{uuid}`. |
| `created` | `TIMESTAMPTZ` | `NOT NULL` | |
...
Shared image storage for all binary image data (player avatars, club logos, match photos, comment images). Entities reference images via `image_id UUID` foreign keys.
...
**Upload flow:** `POST /api/images` accepts multipart/form-data with an image file. The server validates the file type, resizes to 800×800 max via `sharp`, converts to WebP, and inserts into this table. Returns the UUID. The caller then stores the UUID as `image_id` on the relevant entity (`player`, `clubs`, `season_matches`, `match_comments`).
...
| `images` → `match_comments` | 1:N | Comment inline image |
