...
- Tests live alongside the modules they test (e.g. `app/lib/pyramid.test.ts`)
...
- Tests live next to the module they test (e.g. `app/lib/db/auth.test.ts` beside `auth.ts`)
...
│   ├── pyramid.ts              # Challenge rules, rankings
│   ├── pyramid.test.ts         # Unit tests for pyramid logic
│   ├── scoring.ts              # Winner determination
│   ├── scoring.test.ts         # Unit tests for scoring
│   └── ...
├── app/lib/db/                 # Database repository layer
│   ├── auth.ts                 # Auth repository functions
│   ├── auth.test.ts            # DB integration tests for auth
│   ├── smoke.test.ts           # Schema verification
│   ├── test-helpers.ts         # Transaction isolation helpers
