...
Components that import server actions (files with `"use server"`) will crash in Storybook's Vitest browser runner because the action's import chain pulls in Node.js-only modules (`next/cache`, `next/headers`, `postgres`, `crypto`).
