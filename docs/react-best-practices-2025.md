...
React Compiler 1.0 (stable, shipping at Meta) automatically memoizes components and hooks at build time.
...
1. **Primitives** (headless): Unstyled, accessible base components (Radix, React Aria, Headless UI)
2. **Design system**: Styled primitives themed to your brand (shadcn/ui pattern)
...
1. **Unit tests** (Vitest): Pure functions, utilities, hooks — fast, isolated
2. **Component tests** (Storybook interaction tests): Visual + behavioral tests in the browser — play functions simulate user interaction
...
- Use `play` functions with `@storybook/test` (replaces `@storybook/jest` + `@storybook/testing-library`)
- Tests run in the browser — same environment as development
- Combine with a11y addon for accessibility testing per story
- Test runner powered by Playwright for CI
...
- [ ] Unit tests for pure business logic (pyramid calculations, utils)
...
- [ ] E2E tests cover critical user flows (login, challenge, match result)
