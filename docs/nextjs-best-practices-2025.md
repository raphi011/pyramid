...
- **`default.tsx` required** as fallback when a slot has no matching segment
...
// Child page
export const metadata: Metadata = {
  title: 'TC Mödling',  // Renders as "TC Mödling | Pyramid"
}
```
...
- Enable **standalone output**: `output: 'standalone'` in `next.config.js`
...
- **Bundle analysis**: Run `@next/bundle-analyzer` to identify bloat
...
- **Don't configure webpack** unless absolutely necessary — prefer Next.js built-in tooling
...
- [ ] No unnecessary webpack customization
